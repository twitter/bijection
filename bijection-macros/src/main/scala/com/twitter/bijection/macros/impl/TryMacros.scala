...
private[bijection] object TryMacros {
  def fastAttempt[A, B](c: Context)(b: c.Expr[B])(inv: c.Expr[A])(
      implicit A: c.WeakTypeTag[A]): c.Expr[Try[A]] = {
    import c.universe._
    c.Expr[scala.util.Try[A]](q"""(try { _root_.scala.util.Success($inv) }
  ...
  def fastTry[T](c: Context)(toEval: c.Expr[T])(implicit T: c.WeakTypeTag[T]): c.Expr[Try[T]] = {
    import c.universe._
    c.Expr[scala.util.Try[T]](q"""(try { _root_.scala.util.Success($toEval) }
