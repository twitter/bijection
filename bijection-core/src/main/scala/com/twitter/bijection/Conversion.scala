...
trait SuperLowPriorityConversion extends CrazyLowPriorityConversion {
  implicit def fromInjection[A, B](implicit fn: Injection[A, B]) =
    new Conversion[A, B] {
      def apply(a: A) = fn(a)
    }
}
...
trait LowPriorityConversion extends NotSuperLowPriorityConversion {
  implicit def fromBijection[A, B](implicit fn: ImplicitBijection[A, B]) =
    new Conversion[A, B] {
...
object Conversion extends LowPriorityConversion {
  ...
  implicit def fromFunction[A, B](implicit fn: Function1[A, B]) =
    new Conversion[A, B] {
      def apply(a: A) = fn(a)
    }
}
