...
trait CollectionInjections extends StringInjections {
  ...
  implicit def optionInjection[A, B](implicit inj: Injection[A, B]): Injection[Option[A], Option[B]] = new AbstractInjection[Option[A], Option[B]] {
  ...
  implicit def option2List[V1, V2](implicit inj: Injection[V1, V2]): Injection[Option[V1], List[V2]] =
  ...
  implicit def map2Set[K1, V1, V2](implicit inj: Injection[(K1, V1), V2]): Injection[Map[K1, V1], Set[V2]] = toContainer[(K1, V1), V2, Map[K1, V1], Set[V2]]{ _.size == _.size }

  implicit def set2List[V1, V2](implicit inj: Injection[V1, V2]): Injection[Set[V1], List[V2]] = toContainer[V1, V2, Set[V1], List[V2]] { _.size == _.size }

  implicit def set2Vector[V1, V2](implicit inj: Injection[V1, V2]): Injection[Set[V1], Vector[V2]] = toContainer[V1, V2, Set[V1], Vector[V2]] { _.size == _.size }

  // This injection is always correct as long as all the items are inverted
  implicit def list2List[V1, V2](implicit inj: Injection[V1, V2]): Injection[List[V1], List[V2]] = toContainer[V1, V2, List[V1], List[V2]] { (_, _) => true }

  // This injection is always correct as long as all the items are inverted
  implicit def set2Set[V1, V2](implicit inj: Injection[V1, V2]): Injection[Set[V1], Set[V2]] = toContainer[V1, V2, Set[V1], Set[V2]] { (_, _) => true }
